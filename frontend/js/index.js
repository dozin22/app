...
document.addEventListener('DOMContentLoaded', () => {
  ...
  initializePage({ name, position, team });
});
...
function initializePage(userInfo) {
  // 사용자 정보 표시
  document.getElementById('user-info').innerText = `${userInfo.team} ${userInfo.name} ${userInfo.position} 님`;
...
// 로그아웃 (기존 코드와 동일)
function logout() {
  localStorage.clear(); // 모든 localStorage 아이템을 한번에 삭제
  alert("로그아웃 되었습니다.");
  window.location.href = "login.html";
}